...
# Trail / motion settings
MAX_TRAIL_POINTS = 250000
TRAIL_SAMPLE_DISTANCE = 1.0055
CAMERA_DOLLY_SPEED = 1.095


SPAWN_FORWARD_DISTANCE = 15.5
...
class DustScene(pyglet.window.Window):
    def __init__(self):
        config = gl.Config(
            major_version=3,
            minor_version=3,
        ...
        super().__init__(
            WIDTH,
            HEIGHT,
            caption="Signal Fireflies + Trails",
            resizable=True,
            vsync=True,
            config=config,
        )
    ...
    def setup_osc(self):
        ...
        self.osc_dispatcher.map(OSC_ADDRESS, self.osc_wave_handler)
        ...
        print(f"OSC listening on udp://{OSC_IP}:{OSC_PORT}{OSC_ADDRESS}")

    def osc_wave_handler(self, address, *osc_args):
        try:
            values = [float(v) for v in osc_args]
        except (TypeError, ValueError):
            return

        if len(values) == 6:
            x, y, r, g, b, duration = values
            z = 0.0
        elif len(values) == 7:
            x, y, z, r, g, b, duration = values
        else:
            return

        color = np.array([r, g, b], dtype="f4")
        if np.max(color) > 1.0:
            color /= 255.0
        color = np.clip(color, 0.0, 1.0)

        signal_coords = np.array([x, y, z], dtype="f4")
        duration = max(0.03, float(duration))

        self.osc_queue.put((signal_coords, color, duration))



    def process_osc_queue(self, max_messages=64):
        for _ in range(max_messages):
            try:
                signal_coords, color, duration = self.osc_queue.get_nowait()
            except queue.Empty:
                break

            origin = self.signal_to_camera_spawn_world(signal_coords)
            self.spawn_flyer(origin, color, duration)

    def setup_video_background(self):
        self.video_player = pyglet.media.Player()
        # source = pyglet.media.load(VIDEO_PATH, streaming=True)
        source = pyglet.media.load(VIDEO_PATH, decoder=ffmpeg_decoder, streaming=True)
    ...
    #     eye = np.array(
    #         [
    #             0.45 * math.sin(self.time * 0.08),
    #             1.4 + 0.22 * math.cos(self.time * 0.11),
    #             CAMERA_DISTANCE - self.time * CAMERA_DOLLY_SPEED,
    #         ],
    #         dtype="f4",
    #     )
    #     target = np.array([0.0, 2.0, 0.0], dtype="f4")
    ...
    def get_camera_state(self):
        ...
        target = np.array([0.0, 0.6, eye[2] + 6.0], dtype="f4")
        up_hint = np.array([0.0, 1.0, 0.0], dtype="f4")
    ...
    #     eye, _, _, forward, right, true_up = self.get_camera_state()
    #     x, y, z = signal_coords
    #
    #     plane_center = eye + forward * SPAWN_FORWARD_DISTANCE
    #     world = (
    #             plane_center
    #             + right * (x * SPAWN_X_SCALE)
    #             + true_up * (y * SPAWN_Y_SCALE)
    #             + forward * (z * SPAWN_Z_SCALE)
    #     )
    #     return world.astype("f4")
    def signal_to_camera_spawn_world(self, signal_coords):
        eye, _, _, forward, right, _ = self.get_camera_state()
        x, y, z = signal_coords
        ...
        spawn_center = np.array(
            ...
            dtype="f4",
        ) + ground_forward * SPAWN_FORWARD_DISTANCE

        world = (
                spawn_center
                + ground_right * (x * SPAWN_X_SCALE)
                + world_up * (y * SPAWN_Y_SCALE)
                + ground_forward * (z * SPAWN_Z_SCALE)
        )
        return world.astype("f4")
    def screen_to_world_on_camera_plane(self, x, y, forward_distance=SPAWN_FORWARD_DISTANCE):
        x = float(np.clip(x, 0, self.width))
        y = float(np.clip(y, 0, self.height))

        x_ndc = (2.0 * x / self.width) - 1.0
        y_ndc = (2.0 * y / self.height) - 1.0
    ...
    def screen_to_world_on_plane(self, x, y, plane_z=SOURCE_PLANE_Z):
        x = float(np.clip(x, 0, self.width))
        y = float(np.clip(y, 0, self.height))
    ...
    def spawn_flyer(self, origin, color, duration):
        ...
        flyer = {
            ...
            "lifetime": 0.9 + 1.8 * strength + self.rng.uniform(0.0, 0.4),
            "p0": p0.astype("f4"),
            "p1": p1.astype("f4"),
            "p2": p2.astype("f4"),
            "p3": p3.astype("f4"),
    ...
    def append_trail_point(self, pos, color, progress, phase):
        ...
        if len(self.trail_points) > MAX_TRAIL_POINTS:
            drop = len(self.trail_points) - MAX_TRAIL_POINTS
    ...
    def update_flyers(self):
        ...
        for flyer in self.active_flyers:
            ...
            if flyer["last_pos"] is None:
                ...
                if np.linalg.norm(pos - flyer["last_pos"]) >= TRAIL_SAMPLE_DISTANCE:
                    need_sample = True
